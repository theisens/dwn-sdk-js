...
describe("General JWS Sign/Verify", () => {
  ...
  it("should not verify the same signature more than once", async () => {
    ...
    const { privateJwk: privateJwkEd25519, publicJwk: publicJwkEd25519 } =
      await Ed25519.generateKeyPair();
    const { privateJwk: privateJwkSecp256k1, publicJwk: publicJwkSecp256k1 } =
    ...
    const cacheSetSpy = sinon.spy(
      (GeneralJwsVerifier as any).singleton.cache,
      "set"
    );
